...
# Définir df_valide en dehors des tests
...
test_that("calcul_distribution_age calcule correctement les quantiles d'âge", {
  ...
  ages <- c(45, 35, 55, 65, 25)  # Correct en 2025 # Âges calculés pour les quantiles
  expect_equal(quantiles_meme_date, quantile(c(44, 44, 44), probs = c(0, 0.25, 0.5, 0.75, 1)))
})

# Test 2 : Vérifie que la fonction gère correctement un dataframe avec des dates de naissance identiques
test_that("calcul_distribution_age calcule correctement les quantiles pour des dates identiques", {
...
test_that("calcul_distribution_age gère correctement les entrées invalides", {
  df_invalide <- df_valide[, -which(names(df_valide) == "Date.de.naissance")]  # Supprime la colonne Date.de.naissance
